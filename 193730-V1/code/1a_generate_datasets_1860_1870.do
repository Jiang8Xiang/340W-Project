...
*	1a. Compile the historical wealth gap series.
	...
	Distribute household head wealth to all household members: *_eq
	
	Authors: Chi Hyun Kim
...
*-------------------------------------------------------------------------------------------------------------
	...
	*/
	use "$micro_census/raw/CensusData1860FullRegion.dta", clear
	...
	* Import data on national wealth
	...
	/*Share of white with zero wealth, some robustness checks*/
	sum zerototwealth_eq if race != 2  & bpl<60 
	global ZEROWEALTH1860_nonblack_nat_eq = `r(mean)'
	sum zerototwealth_eq if race == 1  & bpl<60 
	global ZEROWEALTH1860_white_nat_eq = `r(mean)'	

	save "$micro_census_jep/CensusData1860.dta", replace
	/************************************************************************************
	Store per capita wealth by state for comparison to "Wealth, Debt, and Taxation" report
	**************************************************************************************/
	tempfile wealthbystate1860
	
	preserve
	use $population/clean_county_year_black_white_population_1850_1940.dta, clear
	keep if year==1860
	collapse (sum) population1860= totpop , by(statefip state_name)
	tempfile pop1860
	save `pop1860'
	restore
	
	preserve
	collapse (sum) wealth1860 = wealth , by(statefip) 
	merge 1:1 statefip using `pop1860'
	drop _merge
	gen wealthpp1860 = wealth1860/population1860
	save `wealthbystate1860', replace
	restore
...
*-------------------------------------------------------------------------------------------------------------
	...
	*/
	...
	use "$micro_census/raw/CensusData1870FullRegion.dta", clear
 ...
 /* 
	...
	* Import data on national wealth
	...
	sum zerototwealth_eq if race != 2  & bpl<60 
	global ZEROWEALTH1870_nonblack_nat_eq = `r(mean)'
	sum zerototwealth_eq if race == 1  & bpl<60 
	global ZEROWEALTH1870_white_nat_eq = `r(mean)'
	
	save "$micro_census_jep/CensusData1870.dta", replace
