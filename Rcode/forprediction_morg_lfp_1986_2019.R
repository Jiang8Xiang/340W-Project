...
dataall <- read_dta(paste0(data_directory, "forpredictionmorg_eventstudy_fewvars_1986_2019_kids.dta"))
...
gc()
#load(file=paste0(Rdata_directory,"full_sample_1986_2019_lfp_child_less_5.RData"))
load(file=paste0(data_directory,"full_sample_1986_2019_lfp_child_less_5.RData"))




#load(file=paste0(Rdata_directory,"prediction_model_1986_2019_switch_lfp_number_child_less_5.RData"))
...
i <- a
#saveRDS(datatemp, paste0(temporary_directory, "CPSmorg_MLpredprobs_",i,".rds"))
saveRDS(datatemp, paste0(data_directory, "CPSmorg_MLpredprobs_",i,".rds"))
...
for (i in (a+1):b)  {
  ...
  #saveRDS(datatemp2, paste0(temporary_directory, "CPSmorg_MLpredprobs_",i,".rds"))
  saveRDS(datatemp2, paste0(data_directory, "CPSmorg_MLpredprobs_",i,".rds"))
  rm(datatemp2)
  print(i)
  gc()
}
...
for (i in c(a:b)){
  print(i)
  #temp <- readRDS(paste0(temporary_directory, "CPSmorg_MLpredprobs_",i,".rds"))
  temp <- readRDS(paste0(data_directory, "CPSmorg_MLpredprobs_",i,".rds"))
  temp <- subset(temp, select = c(rowid, boost.preds))
  data_final <- dplyr::bind_rows(data_final, temp)
}

#data_directory <- "/Users/davidzentlermunro/Dropbox/MinimumWageParticipation/REPL_pkg/data/"
write.dta(data_final, paste0(data_directory, "CPSmorg_MLpredprobsonly_1986_2019_lfp.dta"))
