...
dataall <- read_dta(paste0(data_directory, "fortraining_fewvars_IPUMS_basic_eventstudy_1986_2019_ML_LFP.dta"))
...
i <- a
saveRDS(datatemp, paste0(data_directory, "CPSbasic_DCpredprobs_",i,".rds"))
print(i)
rm(datatemp)
for (i in (a+1):b)  {
  gc()
  ...
  #saveRDS(datatemp2, paste0(temporary_directory, "CPSbasic_DCpredprobs_",i,".rds"))
  saveRDS(datatemp2, paste0(data_directory, "CPSbasic_DCpredprobs_",i,".rds"))
  rm(datatemp2)
  print(i)
  gc()
}
...
for (i in c(a:b)){
  print(i)
  #temp <- readRDS(paste0(temporary_directory, "CPSbasic_DCpredprobs_",i,".rds"))
  temp <- readRDS(paste0(data_directory, "CPSbasic_DCpredprobs_",i,".rds"))
  temp <- subset(temp, select = c(rowid, boost.preds))
  data_final <- dplyr::bind_rows(data_final, temp)
}

#data_directory <- "/Users/davidzentlermunro/Dropbox/MinimumWageParticipation/REPL_pkg/data/"
write.dta(data_final, paste0(data_directory, "CPSbasic_MLpredprobsonly_1986_2019_lfp.dta"))
